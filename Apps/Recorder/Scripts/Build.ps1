...
function Resolve-MSBuild {
  ...
  if ($UserProvided -and (Test-Path $UserProvided)) { return (Resolve-Path $UserProvided).Path }
  $vswhere = "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe"
  if (Test-Path $vswhere) {
    try {
      $msbuild = & $vswhere -latest -requires Microsoft.Component.MSBuild -find "MSBuild\**\Bin\MSBuild.exe" | Select-Object -First 1
      if ($msbuild -and (Test-Path $msbuild)) { return (Resolve-Path $msbuild).Path }
    } catch {}
  }
  $candidates = @(
...
function Resolve-ISCC {
  ...
  try {
    ...
    if ($Diag) { Write-Host "where.exe iscc.exe =>`n$where" }
    if ($where) {
      foreach ($w in ($where -split "`r?`n")) {
        if (Test-Path $w) { return (Resolve-Path $w).Path }
      }
    }
  } catch {}

  # 3) Common install locations (including your earlier Tools dir)
  $candidates = @(
  ...
  foreach ($rp in $regPaths) {
    try {
      ...
      foreach ($it in $items) {
        $pathsToTry = @()
        if ($it.InstallLocation) { $pathsToTry += (Join-Path $it.InstallLocation "ISCC.exe") }
        if ($it.DisplayIcon)     { $pathsToTry += $it.DisplayIcon }
        if ($it.UninstallString) {
          $dir = Split-Path ($it.UninstallString -replace '"','') -ErrorAction SilentlyContinue
          if ($dir) { $pathsToTry += (Join-Path $dir "ISCC.exe") }
        }
...
# Paths (script at ...\Apps\Recorder\Scripts\Build.ps1)
$scriptsDir  = Split-Path -Parent $PSCommandPath
$projectDir  = Split-Path -Parent $scriptsDir              # ...\Apps\Recorder
$appsDir     = Split-Path -Parent $projectDir              # ...\Apps
$repoRoot    = Split-Path -Parent $appsDir                 # repo root
$projectFile = Join-Path $projectDir 'Recorder.csproj'
$issFile     = Join-Path $scriptsDir  'Recorder.iss'       # expect ISS here
...
if (-not (Test-Path $exePath)) {
  ...
  if (Test-Path $exeAlt) { $exePath = $exeAlt }
}
$exeDir = if (Test-Path $exePath) { Split-Path -Parent $exePath } else { $outDir }

# Staging
Write-Header "Staging build output and dependencies"
if ([string]::IsNullOrWhiteSpace($StageDir)) { $StageDir = Join-Path $projectDir 'Stage' }
