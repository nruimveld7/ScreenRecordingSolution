...
namespace ScreenRecorderTray
{
    public sealed class RecorderManager : IDisposable
    {
        ...
        public RecorderManager(NotifyIcon tray)
        {
            ...
            _iniWatcher = new FileSystemWatcher(
                Path.GetDirectoryName(_iniPath) ?? ".",
                Path.GetFileName(_iniPath) ?? "recorder.ini");
            _iniWatcher.NotifyFilter = NotifyFilters.LastWrite | NotifyFilters.Size | NotifyFilters.FileName;
            _iniWatcher.Changed += delegate { DebouncedRestart(); };
            _iniWatcher.Renamed += delegate { DebouncedRestart(); };
            _iniWatcher.EnableRaisingEvents = true;

            SystemEvents.DisplaySettingsChanged += delegate { DebouncedRestart(); };

            AppDomain.CurrentDomain.ProcessExit += delegate { try { ForceKillFfmpegTree(); } catch { } };
        }

        public Task StartAsync() { return RestartAsync(); }
        ...
        private void StartFfmpeg()
        {
            ...
            if (string.IsNullOrWhiteSpace(outPattern)) outPattern = "desktop_%Y%m%d_%H%M%S.mkv";
            ...
            var screens = Screen.AllScreens
                ...
                .ToArray();
            if (screens.Length == 0) throw new InvalidOperationException("No monitors detected.");
            ...
            else
            {
                ...
                string inputs = "";
                for (int i = 0; i < screens.Length; i++) inputs += "[" + i + ":v]";
                filter =
                    inputs + "xstack=inputs=" + screens.Length + ":layout=" + layout + ":fill=black[stack];" +
                    "[stack]pad=ceil(iw/2)*2:ceil(ih/2)*2[stackp];" +
                    "[stackp]fps=" + _cfg.Fps + "[stackf];" +
                    "[stackf]format=yuv420p[final]";
            }
            ...
            lock (_gate)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(e.Data)) RecorderLogger.Log("[ffmpeg] " + e.Data);
                };
                ...
                _job = new WinJob();
                try { if (!_job.Assign(_ffmpeg)) RecorderLogger.Log("Job assign skipped: process already in a job or access denied."); }
                catch (Exception ex) { RecorderLogger.Log("Job assign failed: " + ex.Message); }
            }

            _tray.Text = "Screen Recorder (recording)";

            if (_housekeep != null) _housekeep.Dispose();
            _housekeep = new System.Threading.Timer(_ => { try { Housekeep(); } catch { } }, null, 60000, 60000);

            if (_guard != null) _guard.Dispose();
            ...
            _guard = new System.Threading.Timer(_ =>
            {
                try
                {
                    ...
                    if (p == null || p.HasExited) ScheduleDelayedRestart();
                }
                catch { }
        ...
        private void ScheduleDelayedRestart()
        {
            try
            {
                ...
                if (_restartTimer != null) _restartTimer.Dispose();
        ...
        {
            if (string.IsNullOrEmpty(_currentOutDir) || !Directory.Exists(_currentOutDir)) return;
            ...
            {
                var fi = ordered[i];
                if (!IsFileStable(fi.FullName)) continue;
        ...
        private static void TryDelete(string path)
        {
            ...
            catch (Exception ex) { RecorderLogger.Log("Delete failed: " + path + " :: " + ex.Message); }
        }
        ...
        private string ResolveRecordingUserLabel(string filePath)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(recordRoot) &&
                    fullDir.StartsWith(recordRoot, StringComparison.OrdinalIgnoreCase))
                {
        ...
        {
            if (_housekeep != null) { try { _housekeep.Dispose(); } catch { } _housekeep = null; }
            if (_guard != null) { try { _guard.Dispose(); } catch { } _guard = null; }
            if (_restartTimer != null) { try { _restartTimer.Dispose(); } catch { } _restartTimer = null; }

            Process p = null;
            WinJob job = null;
            lock (_gate)
            {
                if (_runCts != null) { _runCts.Cancel(); _runCts.Dispose(); _runCts = null; }
                p = _ffmpeg;
                job = _job;
                _ffmpeg = null;
                _job = null;
            ...
            if (p != null)
            {
                try
                {
                    if (!p.HasExited)
                    {
                        try { if (p.StartInfo.RedirectStandardInput) { p.StandardInput.WriteLine("q"); p.StandardInput.Flush(); } } catch { }
        ...
        {
            _iniWatcher.Dispose();
            try { StopInternalAsync().Wait(2000); } catch { }
            if (_job != null) { try { _job.Dispose(); } catch { } _job = null; }
            if (_debounce != null) { try { _debounce.Dispose(); } catch { } _debounce = null; }
            if (_housekeep != null) { try { _housekeep.Dispose(); } catch { } _housekeep = null; }
            if (_guard != null) { try { _guard.Dispose(); } catch { } _guard = null; }
            if (_restartTimer != null) { try { _restartTimer.Dispose(); } catch { } _restartTimer = null; }
        }
    ...
    {
        public string RecordDir, OutputPattern, RecordSubdirTemplate, UploadUrl, UploadToken, SystemName, FfmpegPath, Encoder;
        public int SegmentSeconds, Fps, KeepLocal, RetryDelaySeconds, Libx264Crf, ProbeSeconds;
        public bool Verbose;
    }
    ...
    internal static class Identity
    {
        public static Dictionary<string, string> BuildDict()
        {
            ...
            try { dict["session"] = San(Process.GetCurrentProcess().SessionId.ToString()); }
    ...
    internal static class Template
    {
        public static string Apply(string tpl, Dictionary<string, string> vals, string label)
        {
            ...
            {
                return tpl
                    .Replace("{username}", vals["username"])
                    .Replace("{session}", vals["session"])
                    .Replace("{host}", vals["host"]);
            }
            catch
            {
                throw new InvalidOperationException(
                    "Bad token in " + label + ". Allowed: {username}, {session}, {host}");
            }
        }
    }
}
